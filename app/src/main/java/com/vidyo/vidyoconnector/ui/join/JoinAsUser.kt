...
@Composable
fun JoinAsUser(model: JoinViewModel) {
...
private fun FormFields(modifier: Modifier = Modifier, model: JoinViewModel) {
    Column(
        ...
        Spacer(modifier = Modifier.height(16.dp))
        VcDefaultTextField(
            label = { Text(text = stringResource(R.string.join_username)) },
            singleLine = true,
        ...
        Spacer(modifier = Modifier.height(16.dp))
        VcDefaultTextField(
            label = { Text(text = stringResource(R.string.join_password)) },
            singleLine = true,
...
private fun Buttons(modifier: Modifier = Modifier, model: JoinViewModel) {
    ...
    val state = conference.conference.collectAsState()
    ...
    Column(modifier = modifier) {
        ...
        VcGreenButton(
            ...
            enabled = valid && !state.value.state.isActive,
            modifier = defaultModifier.fillMaxWidth(),
        ) {
            Text(text = stringResource(R.string.join_joinAsUser).replace("\n", " "))
        }
        ...
        Row(modifier = Modifier.fillMaxWidth()) {
            ...
            VcTextButton(
                onClick = { model.type.value = JoinContent.Guest },
                modifier = defaultModifier.weight(1f),
            ) {
                Text(text = stringResource(R.string.join_joinAsGuest), textAlign = TextAlign.Center)
            }
        }
    }
}
